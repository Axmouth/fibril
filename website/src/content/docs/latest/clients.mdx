...
```python
sub = await client.subscribe("email.send").group("workers").prefetch(32).sub_manual_ack()
...
```python
sub = client.subscribe("email.send").group("workers").prefetch(32).sub_manual_ack()
...
sub = await client.stream("events").durable("analytics").filter("region", "eu-*").sub_manual_ack()
...
sub = client.stream("events").durable("analytics").filter("region", "eu-*").sub_manual_ack()
...
with BlockingClient.connect("127.0.0.1:9876") as client:
    ...
    sub = client.subscribe("email.send").group("workers").sub_manual_ack()
    for msg in sub:
