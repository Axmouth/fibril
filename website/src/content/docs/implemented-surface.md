...
| Resource incarnation | Partial | Atomic creation and retirement identities bind recovery and stale deletion checks. Explicit version-two enrollment withholds serving until preparation and activation; ordinary enrollment and legacy migration remain rollout gates |
...
| Bounded queue recovery worker | Partial | Explicitly enrolled queues collect witnesses, select a verified source, resume transfer and activate with bounded retries. Initial enrollment, legacy/stream origins, replacement of unavailable pending candidates and recovery-data reclamation remain gates |
