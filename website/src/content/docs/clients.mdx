---
title: Client usage
description: Use the Rust, TypeScript, Python, and Go clients to publish, consume, and settle messages.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Fibril has Rust, TypeScript, Python, and Go clients for the same core broker surface:
...
Parity note: all four clients cover the branch's partition topology,
...
Python, and Go clients mirror its behavior. The Python client also ships a
...
channels, so it has no separate async and sync split.
...
- **Go**: Go 1.23 or newer. No third-party dependencies.
...
```go
import fibril "github.com/Axmouth/fibril/clients/go"

client, err := fibril.Dial("127.0.0.1:9876", fibril.ClientOptions{
...
```go
client, err := fibril.Dial("broker.internal:9876", fibril.ClientOptions{
    Auth: &fibril.Auth{Username: "fibril", Password: "fibril"},
...
publisher.Publish(fibril.Text("hello"))

offset, err := publisher.PublishConfirmed(fibril.Text("needs an offset"))

confirmation, err := publisher.PublishWithConfirmation(fibril.Text("pipeline me"))
pipelinedOffset, err := confirmation.Confirmed()
```
...
```go
publisher.PublishDelayed(fibril.Text("send later"), 30*time.Second)

offset, err := publisher.PublishDelayedConfirmed(
...
publisher.Publish(fibril.Text("plain text").WithHeader("x-trace-id", "abc123"))

publisher.Publish(fibril.Raw([]byte{1, 2, 3}))
```
...
```go
group := "workers"
sub, err := client.SubscribeTopic("email.send", &group, 32, false)
...
`retry()` requeues immediately. Use `retry_after(..)` in Rust and Python, `retryAfter(..)` in TypeScript, or `RetryAfter(..)` in Go for delayed retry.
...
group := "workers"
maxRetries := uint32(3)
client.DeclareQueue(fibril.DeclareQueue{
    Topic:         "email.send",
    Group:         &group,
...
publisher.PublishConfirmed(job)
...
```go
sub, err := client.SubscribeTopic("metrics", nil, 128, true)
...
client.DeclarePlexus(fibril.DeclarePlexus{
    Topic:          "events",
...
// Publish uses the normal publisher.
client.Publisher("events").Publish(fibril.Text("hello"))

durable := "analytics"
sub, err := client.SubscribeStreamTopic("events", fibril.StreamSubscribeOptions{
    DurableName: &durable,
...
sub, err := routing.SubscribePattern("events.*", fibril.PatternSubscribeOptions{})
