...
| Publish and subscribe | Available | Custom TCP protocol with Rust, TypeScript, Python, and Go clients |
...
| Delayed publish | Available | Broker path and Rust, TypeScript, Python, and Go client methods are wired |
...
| Plexus streams | Available | Fan-out channel type: every subscriber receives every record, durable named cursors, per-stream durability tiers, partitioning with client-side fan-in, and header filters. Rust, TypeScript, Python, and Go clients |
| Wildcard subscribe | Available | Opt-in `client.routing()` subscribes to every queue or stream whose topic matches a `*`-glob and auto-attaches channels that start matching later, driven by a live cluster catalogue. Client-side only. Rust, TypeScript, Python, and Go clients |
...
| Exclusive consumer groups | Partial | Rust, TypeScript, Python, and Go client opt-in for one active consumer per partition, with sticky assignment and cross-broker coordinator wiring |
