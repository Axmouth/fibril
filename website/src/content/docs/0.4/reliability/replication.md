...
slug: 0.4/reliability/replication
---
...
mode](/0.4/concepts/clustering/). Standalone brokers own every queue and do
not replicate.
...
latency. See the [configuration](/0.4/configuration/) replication settings.
...
* Cross-broker replication-lag aggregation into a single cluster view is still
  ...
  [admin queues page](/0.4/admin-dashboard/).

## See also

* [Clustering](/0.4/concepts/clustering/) for ownership, epochs, and coordination modes.
* [Recovery quarantine](/0.4/reliability/recovery-quarantine/) for how a node handles a damaged log on restart.
* [Configuration](/0.4/configuration/) for the replication and durability settings.
* [Project status](/0.4/status/) and [implemented surface](/0.4/implemented-surface/) for what is wired.
