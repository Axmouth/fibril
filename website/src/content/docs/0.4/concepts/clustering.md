...
* **Coordinated (Ganglion mode).** Each broker runs an embedded coordinator and
  ...
  mode = "ganglion"` (see [configuration](/0.4/configuration/)).
...
[topology page](/0.4/admin-dashboard/) and through `fibrilctl topology`.
...
* [Replication](/0.4/reliability/replication/) for follower catch-up and replica-durable confirms.
* [Recovery quarantine](/0.4/reliability/recovery-quarantine/) for damaged-log handling on restart.
* [Partitioned queues](/0.4/concepts/consumer-groups/) and [partition routing](/0.4/development/partition-routing/) for how clients address partitions.
* [Configuration](/0.4/configuration/) for coordination settings.
