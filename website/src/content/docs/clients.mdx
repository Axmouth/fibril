...
One Python caveat: the fingerprint pin checks the whole presented chain from
Python 3.13, while older Pythons only expose the leaf certificate, so pin the
server certificate there or use `with_tls_ca_path`.
...
`Client` is `IAsyncDisposable`, so `await using var client = ...` also disposes it
