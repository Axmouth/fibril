...
[implemented surface](/0.4/implemented-surface/).
...
These numbers are architecture sanity checks, not a rigorous benchmark suite. Hardware, durability settings, batching, queue depth, workload, and storage behavior all matter. See [benchmarks](/0.4/benchmarks/) for the fuller queue and stream tables.
