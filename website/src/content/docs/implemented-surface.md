...
| Eager failure detection | Experimental | Opt-in cluster runtime policy uses repeated explicit Raft transport failures and a reconnect grace. Success or a fresh heartbeat clears suspicion; heartbeat expiry handles silent failures. Recovery proof and confirmation thresholds remain required; diagnostics are in controller status |
