...
configuration: fix the CA path or the fingerprint), and `TlsConfig` for
client-side option problems. The TypeScript and Python names are
`TlsRequiredByBrokerError` and so on. None of them are retried automatically.
