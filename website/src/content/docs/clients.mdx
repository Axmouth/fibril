...
Fibril reserves `fibril.*` and `stroma.*` headers for system metadata. User code should not set headers with those prefixes. See [metadata policy](/development/metadata-policy/) for the development note.
...
```csharp
await client.DeclareQueueAsync("email.send", new QueueDeclareOptions
{
    Group = "workers",
...
{
    PartitionCount = 4,
    Retention = new StreamRetentionPolicy { MaxRecords = 1_000_000 },
});
...
{
    handle(msg.Text());
    msg.Ack(); // advances the durable cursor
}
```
