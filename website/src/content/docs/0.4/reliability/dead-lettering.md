...
slug: 0.4/reliability/dead-lettering
---
...
Partitioning is currently an internal detail for dead-letter configuration. Admin API responses may include `part`, but request bodies can omit it. Once queue sharding is available, Fibril should choose an available partition for DLQ routing rather than asking operators to pick one here. See [partition routing](/0.4/development/partition-routing/) for the development policy.
