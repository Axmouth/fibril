//! Fibril v1 wire protocol: the message types plus the byte-exact body and frame
//! codec. A leaf crate the clients and the broker both build on, so a client
//! depends only on the wire format, not the server.

pub mod frame;
pub mod helper;
pub mod wire;

/// Default heartbeat interval in seconds, shared by the client and broker.
