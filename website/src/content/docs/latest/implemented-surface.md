...
| Live topology push | Partial | Broker pushes a `TopologyUpdate` to each connection when the routing content changes (not on every heartbeat-driven generation bump); the Rust, TypeScript, and Python clients apply it to their routing cache and ack the generation |
