...
client.DeclareQueue(ctx, fibril.QueueConfig{
    Topic:         "email.send",
    Group:         &group,
    DlqPolicy:     &fibril.DlqPolicy{Kind: fibril.DlqGlobal},
...
```go
partitions := uint32(4)
maxRecords := uint64(1_000_000)
client.DeclarePlexus(ctx, fibril.StreamConfig{
    Topic:          "events",
    PartitionCount: &partitions,
    Retention:      fibril.StreamRetention{RetainRecords: &maxRecords},
})
