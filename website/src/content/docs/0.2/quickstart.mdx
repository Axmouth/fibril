...
For more publishing, subscription, delayed publish, and message encoding examples, see [client usage](/0.2/clients/).

For startup config, runtime settings, and idle queue cleanup options, see [configuration](/0.2/configuration/).

## Try a cluster

To stand up a real multi-broker cluster locally and watch ownership, replication, and failover from the admin dashboard, see [try a cluster with Docker in under a minute](/0.2/concepts/clustering/#try-a-cluster-with-docker). Clustering is experimental.
