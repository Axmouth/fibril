...
async fn inflight_and_ack() -> anyhow::Result<()> {
    ...
    store.append(&topic, 0, &None, b"a").await?;
    store.append(&topic, 0, &None, b"b").await?;

    let msgs = store.fetch_available(&topic, 0, &group, 0, 10).await?;
    assert_eq!(msgs.len(), 2);

    store
        .mark_inflight(&topic, 0, &group, msgs[0].offset, 999999)
        .await?;
    store.ack(&topic, 0, &group, msgs[0].offset).await?;
...
async fn redelivery() -> anyhow::Result<()> {
    ...
    store.append(&topic, 0, &None, b"x").await?;

    let msgs = store.fetch_available(&topic, 0, &group, 0, 10).await?;
    let off = msgs[0].offset;
...
async fn out_of_order_acks() -> anyhow::Result<()> {
    ...
    for i in 0..5 {
        store
            .append(&topic, 0, &None, format!("msg-{i}").as_bytes())
            .await?;
    }
    ...
    let offsets: Vec<_> = msgs2.iter().map(|m| m.offset).collect();
...
async fn redelivery_after_expiration() -> anyhow::Result<()> {
    ...
    let off = store.append(&topic, 0, &None, b"a").await?;
    ...
    assert_eq!(expired.len(), 1);
    assert_eq!(expired[0].offset, off);
...
async fn out_of_order_ack_behavior() -> anyhow::Result<()> {
    ...
    let group = Some("group1".to_string());
...
async fn inflight_messages_not_fetched() -> anyhow::Result<()> {
    ...
    store.append(&topic, 0, &None, b"a").await?; // offset 0
    store.append(&topic, 0, &None, b"b").await?; // offset 1
...
async fn prefix_isolation() -> anyhow::Result<()> {
    ...
    store.append(&t1, 0, &None, b"x").await?;
    store.append(&t2, 0, &None, b"y").await?;
...
async fn cleanup_removes_old_acked_messages() -> anyhow::Result<()> {
    ...
    store.cleanup_topic(&topic, 0, &group).await?;
...
async fn cleanup_blocked_by_other_group() -> anyhow::Result<()> {
    ...
    store.cleanup_topic(&topic, 0, &g2).await?;
...
async fn cleanup_preserves_remaining_offsets() -> anyhow::Result<()> {
    ...
    store.cleanup_topic(&topic, 0, &g).await?;

    let msgs = store.fetch_available(&topic, 0, &g, 0, 10).await?;
    let offsets: Vec<_> = msgs.iter().map(|m| m.offset).collect();
...
async fn append_batch_basic() -> anyhow::Result<()> {
    ...
    let offsets = store.append_batch(&topic, 0, &None, &payloads).await?;
    assert_eq!(offsets, vec![0, 1, 2]);

    let msgs = store
        .fetch_available(&topic, 0, &Some("g".to_string()), 0, 10)
        .await?;
...
async fn append_and_batch_interleave() -> anyhow::Result<()> {
    ...
    let off1 = store.append(&topic, 0, &None, b"x").await?;
    assert_eq!(off1, 0);

    let offs = store
        .append_batch(&topic, 0, &None, &[b"a".to_vec(), b"b".to_vec()])
        .await?;
    ...
    let payloads: Vec<_> = msgs.into_iter().map(|m| m.payload).collect();
...
async fn append_batch_empty() -> anyhow::Result<()> {
    ...
    let offs = store.append_batch(&topic, 0, &None, &[]).await?;
