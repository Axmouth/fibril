...
* **Coordinated (Ganglion mode).** Each broker runs an embedded coordinator and
  ...
  mode = "ganglion"` (see [configuration](/0.3/configuration/)).
...
[topology page](/0.3/admin-dashboard/) and through `fibrilctl topology`.
...
* [Replication](/0.3/reliability/replication/) for follower catch-up and replica-durable confirms.
* [Recovery quarantine](/0.3/reliability/recovery-quarantine/) for damaged-log handling on restart.
* [Partitioned queues](/0.3/concepts/consumer-groups/) and [partition routing](/0.3/development/partition-routing/) for how clients address partitions.
* [Configuration](/0.3/configuration/) for coordination settings.
