...
For the reverse view, use [implemented surface](/0.4/implemented-surface/) to
...
completeness. Per-feature maturity lives in [project status](/0.4/status/).
...
[implemented surface](/0.4/implemented-surface/) inventory, and the
[project status](/0.4/status/) exactly as that release shipped them while the root
...
* A Prometheus `/metrics` endpoint on the admin listener - node-level
  ...
  [monitoring](/0.4/deployment/monitoring/).
* Graceful ownership handoff on drain, so a planned restart moves partition
  ...
  See
  [failure modes](/0.4/reliability/failure-modes/#planned-restart-or-rolling-upgrade).
* TLS on inter-broker replication and coordination connections, with a
  ...
  [TLS across nodes](/0.4/deployment/cluster/#tls-across-nodes).
...
conditions, see [implemented surface](/0.4/implemented-surface/).
...
* Produce repeatable benchmark reports and keep the
  [optimization log](/0.4/development/optimization-log/) current as
...
* Durable broker restart reconciliation. This extends the live-process graceful
  reconnect (see [reconnection grace](/0.4/development/reconnection-grace/))
