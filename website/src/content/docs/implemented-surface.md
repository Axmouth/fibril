...
| First-boot setup mode | Implemented | `setup.mode = true` + no marker serves only a localhost setup page (generate, supply PEMs, or explicit skip), the broker starts after the choice. The choice persists as a config overlay layered below explicit config |
