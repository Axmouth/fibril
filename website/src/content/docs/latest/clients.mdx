...
Current parity note: the Rust client has the branch's partition topology,
owner-redirect, partition-key routing, and exclusive consumer-group surface.
The TypeScript client covers the core single-owner queue API and needs a
cohesive parity pass before it should be treated as cluster/cohort ready.
...
By default, both clients make one automatic reconnect attempt before a new
operation if the previous engine is already closed. Disable that with
`disable_auto_reconnect()` in Rust or `disableAutoReconnect()` in TypeScript.
After a successful resume, both clients send their known subscription metadata
to the broker and read the reconciliation result. Subscriptions that the broker
confirms with `keep` continue on the existing stream. This does not replay
...
`withReconnectReconcilePolicy("restore_client_subscriptions")`.
...
Restored subscriptions may receive a new server subscription id, which both
...
`retry()` requeues immediately. Use `retry_after(..)` in Rust or `retryAfter(..)` in TypeScript for delayed retry.
