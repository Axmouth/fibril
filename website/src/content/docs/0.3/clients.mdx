---
title: Client usage
description: Use the Rust, TypeScript, and Python clients to publish, consume,
  and settle messages.
slug: 0.3/clients
---
...
For reconnect behavior and current limits, see [reconnects](/0.3/reliability/reconnects/).
...
in-flight publish operations. See [reconnects](/0.3/reliability/reconnects/)
...
Fibril reserves `fibril.*` and `stroma.*` headers for system metadata. User code should not set headers with those prefixes. See [metadata policy](/0.3/development/metadata-policy/) for the development note.
...
A [Plexus stream](/0.3/concepts/plexus-streams/) delivers every record to
