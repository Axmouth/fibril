...
```go
publisher.PublishDelayed(payload, 30*time.Second)
publisher.PublishDelayedConfirmed(payload, 30*time.Second)
```
...
Per-message TTL. Rust, TypeScript, and Python bind it to an `expiring` publisher.
Go sets it per message with `WithTTL`:
...
```go
publisher := client.Publisher("rpc.reply")
publisher.Publish(fibril.Text("reply").WithTTL(30 * time.Second))
```
