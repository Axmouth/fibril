...
For reconnect behavior and current limits, see [reconnects](/0.4/reliability/reconnects/).
...
in-flight publish operations. See [reconnects](/0.4/reliability/reconnects/)
...
[TLS across nodes](/0.4/deployment/cluster/#tls-across-nodes) for the broker-side
setup.
...
Fibril reserves `fibril.*` and `stroma.*` headers for system metadata. User code should not set headers with those prefixes. See [metadata policy](/0.4/development/metadata-policy/) for the development note.
...
A [Plexus stream](/0.4/concepts/plexus-streams/) delivers every record to
