...
| Resource incarnation | Partial | New queue/stream catalogue declarations receive an atomic consensus ID; recovery transition digests bind it and stale deletion checks it. Existing/retiring resources remain unverified. Storage binding, accepted history and owner-restart admission remain pending |
| Initial history preparation | Partial | Explicit consensus decisions bind incarnation, assignment and owner instance to fixed history/session IDs. Replica preparation persists storage without writer admission; receipt collection requires the owner and write-count threshold. Automatic enrollment, persisted quorum installation and activation remain pending |
