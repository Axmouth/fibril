...
Confirmed publish waits for the broker to acknowledge the stored message and returns the topic offset.
...
publisher
    .publish_unconfirmed("fire and forget")
    .await?;
```
...
const offset = await publisher.publish("hello");
...
publisher
    .publish_unconfirmed_delayed("send later without confirm", 30u64)
    .await?;
```
...
```ts
const offset = await publisher.publishDelayed("send later", 30_000);

await publisher.publishUnconfirmedDelayed(
  "send later without confirm",
...
Fibril reserves `fibril.*` headers for broker-owned metadata. User code should not set headers with that prefix.
