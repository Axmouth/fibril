...
The dashboard shows retained records and each durable cursor’s distance from the tail. This read-only example has one cursor at the tail and another catching up.

<DashboardDemo view="streams" title="Stream retention and consumer positions" />
