...
| Automatic failover | Partial | Changes involving replicated confirmation persist a pending recovery request and retain the previous assignment/source; automatic recovery and activation await the confirmed-history protocol. Legacy owner-only policy retains its existing failover behavior; explicitly enrolled histories require recovery for assignment changes |
...
| Recovery witness admission | Partial | Explicit calls use fresh authenticated connections and bounded deadlines; collection binds replies to distinct old replicas and the exact transition, preserves the old threshold and rejects contradictions. Activated-history witnesses require the original durable storage receipt; eligible-set thresholds intersect configured writes. Count completion awaits history validation. Automatic collection, transfer and activation remain pending |
...
| Resource incarnation | Partial | New queue/stream catalogue declarations receive an atomic consensus ID; recovery transitions and stale deletion checks bind it. Explicit version-two enrollment withholds serving while retaining placement; deletion retains a retired marker. Existing resources need a verified baseline; automatic enrollment and recovery readmission remain pending |
...
| Queue recovery source selection | Partial | Explicit selection for accepted version-two initial origins verifies complete state artifacts, old witnesses, observed tails and comparisons. Stable snapshot bytes preserve retry/timer/TTL state and release old leases. Crossed tails, legacy baselines, recovered lineage, installation and automatic activation remain pending |
| Queue recovery intent | Partial | Guarded consensus persists the verified source, witness identities, state fingerprints and fresh history/session IDs. Retries retain the first plan and leave the old assignment fenced; staged installation and new-quorum activation remain pending |
| Queue recovery staging | Partial | A fresh plan check authorizes bounded pages into a separate native log; immutable intent, exact retries, restart progress and full payload/state verification preserve the old sealed source. Linux SIGKILL and authenticated TCP tests pass. Staged completion does not authorize replacement or serving |
